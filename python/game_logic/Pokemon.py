...
class Pokemon :
    ...
    def get_needed_exp(self, level, curve):
        if curve == "Fast":
            ...
            if level <= 50:
                ...
                return (level ** 3) * ((1,274 - (1/50)) * (level / 3))
            else:
    ...
    def get_stat_hp(self, level, hp, ev_hp, iv_hp):
        return ((2 * hp + ev_hp/4 + iv_hp) * level) / 100 + level + 10
    ...
    def evolve(self):
        ...
        conn.close()
        self.hpmax = self.get_stat_hp(self.Level, self.hp, self.ev_hp, self.hp_iv)
