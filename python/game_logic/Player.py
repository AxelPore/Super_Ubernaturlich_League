    ...
    async def add_pokemon(self, pokemon):
        async with aiosqlite.connect('database.db') as conn:
            ...
            if len(self.equipe) == 1:
                await conn.execute("UPDATE Equipe SET Pokemon2 = ? WHERE Equipeid = ?", (await conn.lastrowid, self.equipeid))
            elif len(self.equipe) == 2:
                await conn.execute("UPDATE Equipe SET Pokemon3 = ? WHERE Equipeid = ?", (await conn.lastrowid, self.equipeid))
            elif len(self.equipe) == 3:
                await conn.execute("UPDATE Equipe SET Pokemon4 = ? WHERE Equipeid = ?", (await conn.lastrowid, self.equipeid))
            await conn.commit()
            self.pokemon.clear()
            self.equipe.clear()
            pc_cursor = await conn.execute("SELECT Pokedexid, Pokemonid FROM Pokemon WHERE Userid = ?", (self.userid,))
